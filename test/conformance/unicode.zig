...
const word_break_tests = unicode_test_data.word_break_test;
...
test "Unicode 17 default word break conformance" {
    var lines = std.mem.splitScalar(u8, word_break_tests, '\n');
    var line_number: usize = 0;
    while (lines.next()) |raw_line| {
        line_number += 1;
        const comment = std.mem.indexOfScalar(u8, raw_line, '#') orelse raw_line.len;
        ...
        var iterator = try tui.text.WordBreakIterator.init(encoded[0..encoded_len]);
        while (iterator.next()) |boundary| {
            actual[actual_len] = boundary.offset;
            actual_len += 1;
        }
        if (!std.mem.eql(usize, expected[0..expected_len], actual[0..actual_len])) {
            std.debug.print("word break conformance mismatch on data line {d}\n", .{line_number});
            try std.testing.expectEqualSlices(usize, expected[0..expected_len], actual[0..actual_len]);
        }
    }
}
